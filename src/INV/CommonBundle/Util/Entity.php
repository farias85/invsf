<?php
/**
 * Created by PhpStorm.
 * User: Felipe
 * Date: 09/08/2017
 * Time: 11:31
 */

namespace INV\CommonBundle\Util;


class Entity {

    /**
     * CommonBundle
     */
    const ACTIVO_FIJO = "CommonBundle:ActivoFijo";
    const APUNTE = "CommonBundle:Apunte";
    const AUDITORIA = "CommonBundle:Auditoria";
    const CHEQUEO = "CommonBundle:Chequeo";
    const ESTADO = "CommonBundle:Estado";
    const INFORME = "CommonBundle:Informe";
    const LOCAL = "CommonBundle:Local";
    const METADATA = "CommonBundle:Metadata";
    const RESPONSABLE = "CommonBundle:Responsable";
    const REVISION = "CommonBundle:Revision";
    const ROL = "CommonBundle:Rol";
    const SOBRANTE = "CommonBundle:Sobrante";
    const TIPO_ACTIVO = "CommonBundle:TipoActivo";
    const TIPO_RESULTADO = "CommonBundle:TipoResultado";
    const USUARIO = "CommonBundle:Usuario";



}